...
void lab2p2()
{
    ...
    if(cpuid == 0) {
        ...
        //vm_mappages(test_pgtbl, PGSIZE * 10, PGSIZE / 2, mem[1], PTE_R | PTE_W);
        //vm_mappages(test_pgtbl, PGSIZE * 512, PGSIZE - 1, mem[2], PTE_R | PTE_X);
        vm_mappages(test_pgtbl, PGSIZE * 512 * 512, PGSIZE, mem[2], PTE_R | PTE_X);
        vm_mappages(test_pgtbl, MAXVA - PGSIZE, PGSIZE, mem[4], PTE_W);
        vm_print(test_pgtbl);

        printf("\ntest-2\n\n");    
        //vm_mappages(test_pgtbl, 0, PGSIZE, mem[0], PTE_W);
